...
class Interactor(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.modified:
            ...
            # 论文中的公式1，d是Fs odd,c是Fs even
            d = x_odd.mul(torch.exp(self.phi(x_even)))
            c = x_even.mul(torch.exp(self.psi(x_odd)))

            # 论文中的公式2， x_even_update是F‘even，x_odd_update是F’odd
...
class LevelSCINet(nn.Module):
    ...
    def forward(self, x):
        ...
        #permute对向量进行转置，若原来三维向量维度是（4，5，6），
        # 4的下标是0，5的是1，6的是2，permute(0, 2, 1)表示将原来三维向量的维度变为（4，6，5）
...
class SCINet_Tree(nn.Module):
    ...
    def forward(self, x):
        ...
        #到达叶子节点会调用 zip_up_the_pants 函数将结果合并。
...
class SCINet(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        if self.num_decoder_layer > 1:
            self.projection1 = nn.Linear(self.input_len, self.output_len)  #二叉树最后的全连接层
    ...
    def forward(self, x):
        assert self.input_len % (np.power(2, self.num_levels)) == 0  # 将输入长度平均分成两个部分（例如，对于3个等级，32 -> 16 -> 8 -> 4）。
        if self.pe:
        ...
        res1 = x
        x = self.blocks1(x)  # 中间一系列的递归下采样交互操作
        x += res1
        if self.num_decoder_layer == 1:
            x = self.projection1(x)  # 全连接层
        else:
            x = x.permute(0, 2, 1)
