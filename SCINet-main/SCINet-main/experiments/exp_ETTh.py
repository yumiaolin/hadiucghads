...
class Exp_ETTh(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        data_dict = {
            ...
            'IMF-1':Dataset_Custom,
            'FJT_S':Dataset_Custom,
    ...
    def test(self, setting, evaluate=False):
        ...
        if self.args.save:
            ...
            f = open("result_FJT_S_forecast.txt", 'a')
            f.write(setting + "  \n")
    ...
    #接收一个数据集对象、一批输入(batch_x)和相应的输出(batch_y)，
    # 然后使用神经网络模型处理这批数据，并返回预测输出以及预测和真实输出的缩放版本。
    def _process_one_batch_SCINet(self, dataset_object, batch_x, batch_y):
        ...
        u, u_hat, omega = VMD(batch_x, alpha, tau, K, DC, init, tol)
        f_outputs=0
        re_batch_x=0
        for i in range(K):
            x = u[i, :]
            x = x.reshape(batch_x.shape)
            x = torch.from_numpy(x).cuda()
            re_batch_x=re_batch_x+x

        if self.args.stacks == 1:
            outputs = self.model(re_batch_x)
            f_outputs=f_outputs+outputs
        elif self.args.stacks == 2:
            outputs, mid = self.model(re_batch_x)
            f_outputs = f_outputs + outputs
        else:
            print('Error!')
        # for i in range(K):
        ...
        # u, u_hat, omega = VMD(batch_y, alpha, tau, K, DC, init, tol)
        # for i in range(K):
        #     y=u[i,:]
        #     y = y.reshape(batch_y.shape)
        #     batch_y=batch_y+y;
        ...
        # 对输出和真实数据进行缩放，该函数可以反转在数据预处理步骤中应用的任何归一化或缩放。
        # 该函数返回输出(outputs或mid，取决于栈的值)，
        # 以及它们的缩放版本(outputs_scaled和/或mid_scaled)，真实输出(batch_y)及其缩放版本l(batch_y_scaed)。
        # if self.args.inverse:
        outputs_scaled = dataset_object.inverse_transform(f_outputs)
        ...
        if self.args.stacks == 1:
            ...
            return f_outputs, outputs_scaled, 0, 0, batch_y, batch_y_scaled
        elif self.args.stacks == 2:
             # return outputs, outputs_scaled, mid, mid_scaled, batch_y, batch_y_scaled
             return f_outputs, outputs_scaled, mid, mid_scaled, batch_y, batch_y_scaled
        else:
            print('Error!')
